...
Функция ПолучитьСтруктуруПравилКонвертации(Отказ, ИмяПланаОбмена) Экспорт
	
	ДвоичныеДанные = ПолучитьДвоичныеДанныеИзМакетаКонфигурации(Отказ, ИмяПланаОбмена, "ПравилаОбмена");
	...
	// зачитываем правила конвертации
	ЗагрузкаПравилКонвертацииОбъектов = Обработки.ЗагрузкаПравилКонвертацииОбъектов.Создать();
	
	// возвращаемое значение функции
	СтруктураПравилКонвертации = ЗагрузкаПравилКонвертацииОбъектов.ПолучитьСтруктуруПравилОбмена(ИмяВременногоФайла);
	
	Если ЗагрузкаПравилКонвертацииОбъектов.ФлагОшибки Тогда
		Отказ = Истина;
	КонецЕсли;
	...
	Возврат СтруктураПравилКонвертации;
КонецФункции
...
Функция ПолучитьДвоичныеДанныеИзМакетаКонфигурации(Отказ, ИмяПланаОбмена, ИмяМакета)
	...
	Исключение
		...
		СтрокаСообщения = НСтр("ru = 'Ошибка получения макета конфигурации [ИмяМакета] для плана обмена [ИмяПланаОбмена]'");
		СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяМакета]",      ИмяМакета);
