&НаКлиенте
Процедура ЗаполнитьСписокФайлов(ПутьФайла, Знач ЭлементыДерева, ЭлементВерхнегоУровня)
	...
	Если ФайлПеренесенный.ЭтоКаталог() Тогда
		...
		Путь = ФайлПеренесенный.ПолноеИмя + "\";
...
Процедура ПриОткрытии(Отказ)
	...
	Для Каждого ПутьФайла Из СписокИменФайлов Цикл
		ЗаполнитьСписокФайлов(ПутьФайла, ДеревоФайлов.ПолучитьЭлементы(), Истина);
	КонецЦикла;
...
Процедура ДобавитьВыполнить()
	...
	Оповестить("ИмпортКаталоговЗавершен",,);
