	...
	#Если НЕ ВебКлиент Тогда
		...
		Если не КаталогВыгрузки.Существует() Тогда
			...
			Попытка
				...
				СоздатьКаталог(ПапкаДляЭкспорта);
		...
		Успех = ОбойтиДеревоФайлов(ФормаВопроса, ДеревоФайлов, ПапкаДляЭкспорта, Ложь, "", ЕщеНеВстретилиВыгружаемуюПапку, ЧтоСохраняем);
		
		Если Успех Тогда
			СохраняемыйПуть = ПапкаДляЭкспорта;
			
			ИмяПапки = Строка(ЧтоСохраняем) + "\";
			КонецПути = Прав(ПапкаДляЭкспорта, СтрДлина(ИмяПапки));
			
			Если КонецПути = ИмяПапки Тогда
				СохраняемыйПуть = Лев(ПапкаДляЭкспорта, СтрДлина(ПапкаДляЭкспорта) - СтрДлина(ИмяПапки));
			КонецЕсли;
		...
		Для Каждого ЗаписьФайла из ТаблицаФайлов.ПолучитьЭлементы() Цикл
			...
			Если (ЗаписьФайла.Папка <> ЧтоСохраняем) И ЗаписьФайла.ТекущаяВерсия.Пустая() Тогда
				Если РодительскаяПапка <> ЗаписьФайла.Папка Тогда
					БазовыйКаталогСохраненияФайла = БазовыйКаталогСохраненияФайла + ЗаписьФайла.НаименованиеПапки + "\";
				КонецЕсли;
			КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	ПапкаДляЭкспорта = ФайловыеФункцииКлиент.КаталогМоиДокументы() + Строка(ЧтоСохраняем) + "\";
КонецПроцедуры
